...
class Database():
    ...
    def __init__(self,database):
        ...
        if not QUIET: print("{class_name}:{method}:initialized database:{database}".format(method=method,class_name=self.class_name, database=database))    
        return None
    ...
    def createTable(self,**kwargs):
        ...
        if unique_fields:
            ...
            for unique in unique_fields:
                sql_statement += "CREATE UNIQUE INDEX `idx_{index}` ON `{table}` (`{index}`); ".format(table=table_name,index=unique)
            try:
    ...
    def insertORupdate(self,**kwargs):
        ...
        sql_statement = "INSERT or ignore INTO `{table}` (`{key_field}`) VALUES(?) ; ".format(table=table_name,key_field=key_field)
